...
nb_villes = 5
dix_data = data[:,0:nb_villes].clone()
...
decodeur = torch.nn.Sequential(
    torch.nn.Linear(d_latent,nb_villes),
    torch.nn.Sigmoid()
)
...
for N in range(nb_epoch):
    v_idx = np.random.randint(0,nb_villes,d_batch)
...
freq = [0 for i in range(nb_villes)]
perc = [0 for i in range(nb_villes)]

for N in range(5000):
    i = np.random.randint(0,nb_villes)
...
print("Accuracy per class: ",[format(perc[i]/freq[i], '.2f') for i in range(nb_villes)])
