...
data = DataLoader(train_data, shuffle = False, batch_size = 10, collate_fn = train_data.collate)
...
for x,y,z in data:
    ...
    y_pred = torch.nn.utils.rnn.pad_packed_sequence(y_pred,batch_first=True)[0].permute(0,2,1)
    print("iteration numero juste avant la loss: ",i+1)
    loss = loss_fn(y_pred,y)
    print("loss calculée")
    #loss = torch.masked_select(loss,y != 20).mean()
    #optim.zero_grad()
    #loss.backward()
    #optim.step()
    i += 1
...
#BUGGED REGARDER CONVERSATION MATTERMOST POUR DETAILS
